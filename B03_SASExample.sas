  LIBNAME FileIN '[Path to source file]' ACCESS=readonly;   
  LIBNAME FileOUT '[Path to source file]/EstimationResults/SAS' ;
 /*Import the simulated CLSA Dataset*/ 
Proc import out=FileOUT.CLSAExample
datafile="[Path to source file]/CLSASmallExample.csv"
dbms=dlm replace ;
  delimiter=",";
GETNAMES=YES;
DATAROW=2;
guessingrows=max ;    
run;
 /*Format the order of the  categorical variable */  
PROC Format; 
...
RUN;
/*Modify the data to suitable format*/ 
DATA CLSADATA ;
